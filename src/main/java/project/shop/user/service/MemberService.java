...
import project.shop.user.config.SecurityUtil;
import project.shop.user.domain.Member;
import project.shop.user.dto.MemberRequestDto;
import project.shop.global.response.Response;
import project.shop.user.dto.TokenDto;
...
public class MemberService {
    ...
    @Transactional
    public ResponseEntity<?> signUp(MemberRequestDto.SignUp signUp) throws Exception {

        if (memberRepository.findByEmail(signUp.getEmail()).isPresent()){
            throw new Exception("이미 존재하는 이메일입니다.");
        }

        if (!signUp.getPassword().equals(signUp.getCheckedPassword())){
        ...
        Member member = Member.builder()
                ...
                .roles(Collections.singletonList(authType.ROLE_USER.name()))
                .build();
    ...
    public ResponseEntity<?> reissue(MemberRequestDto.Reissue reissue) {
        ...
        // 4. 로그아웃되어 Redis 에 RefreshToken 이 존재하지 않는 경우 처리
    ...
    public ResponseEntity<?> authority() {
        ...
        member.getRoles().add(authType.ROLE_ADMIN.name());
